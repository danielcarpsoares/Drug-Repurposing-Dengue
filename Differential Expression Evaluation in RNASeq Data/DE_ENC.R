...
###Pick only local aligned samples for Encephalon
enclocal <- c(8,14,20,33,39,46)

dataUniqueenc <- dataUnique[,enclocal]

###Set conditions for DE
disease_state <- c("Infected","Infected","Infected",
                   "Control","Control","Control")

coldata <- data.frame(col.names = colnames(dataUniqueenc),
                      group = disease_state)

DESeq2Table <- DESeqDataSetFromMatrix(dataUniqueenc, colData = coldata, design = ~group)
...
write.table(file="genesDEEnc.txt", DESeq2Res, col.names=T, row.names=T, sep="\t", quote=F)
write.table(file="upregEnc.txt", row.names(up), sep = "\t")
write.table(file="downregEnc.txt", row.names(down), sep="\t")